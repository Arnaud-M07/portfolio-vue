  ...
  <section>
    ...
    >
      <div
        ...
        :style="{ display: 'flex', transform: `translateX(${scrollTransforms[index] || '0px'})`, ...scrollerStyle }"
...
const setContainerRef = (el: HTMLDivElement | null, index: number) => {
  if (el) containerRef.value[index] = el;
};

// Tu peux créer la même pour scrollerRef et copyRefs
const setScrollerRef = (el: HTMLDivElement | null, index: number) => {
  if (el) scrollerRef.value[index] = el;
};

const setCopyRef = (el: HTMLSpanElement | null, index: number) => {
  if (el) copyRefs.value[index] = el;
};
